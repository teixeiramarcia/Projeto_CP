...
\\
a80943 & Márcia Filipa Vilela Oliveira Teixeira	
\\
a82522 & Pedro Duarte Cunha e Silva	
\end{tabular}
\end{center}
...
inExpr = either Num bop'
        where
        bop' (x,(e1,e2)) = Bop e1 x e2

outExpr :: Expr -> Either Int (Op,(Expr,Expr))
outExpr (Num x) = i1 (x)
outExpr (Bop e1 x e2) = i2 (x,(e1,e2))
...
opTofun (Op "-") = uncurry (-)
opTofun (Op "*") = uncurry (*)
opTofun (Op "/") = uncurry (div)

calcula :: Expr -> Int
calcula = cataExpr(either id (Cp.ap . (opTofun >< id)))
...
show' = cataExpr(either show (capture.conc.(conc >< id).split (split (p1.p2) (showOp.p1)) (p2.p2)) )

compileAuxOp1 :: Op -> Codigo
compileAuxOp1 (Op "+") = ["ADD"]
compileAuxOp1 (Op "-") = ["SUB"]
compileAuxOp1 (Op "*") = ["MUL"]
compileAuxOp1 (Op "/") = ["DIV"]

compileAuxOp2 :: (Op,(Codigo,Codigo)) -> Codigo
compileAuxOp2 (x,(a,b)) = a ++ b ++ (compileAuxOp1 x)

compileAuxNum :: Int -> Codigo
compileAuxNum x = ["PUSH " ++ show x]

compile :: String -> Codigo
compile = cataExpr (either compileAuxNum compileAuxOp2) . fst . head . readExp
\end{code}
...
dimen  (Unid a) =  (fromIntegral(p1(p1(a))),fromIntegral(p2(p1(a))))
dimen (Comp t a b)   |t==Ht = (p1(dimen a)+p1(dimen b) , p2(dimen a)+p2(dimen b))
                     |t==Vd = (p1(dimen a)+p1(dimen b) , p2(dimen a)+p2(dimen b))
                     |t==Ve = (max (p1(dimen a)) (p1(dimen b)),p2(dimen a)+p2(dimen b))
                     |t==Hb = (p1(dimen a)+p1(dimen b),(max (p2(dimen a)) (p2(dimen b))))
                     |t==H = (p1(dimen a)+p1(dimen b),p2(dimen a)+ ((p2(dimen b))/2))
                     |t==V = (p1(dimen a)+ ((p1(dimen b))/2),p2(dimen a)+p2(dimen b ))

calcOrigins :: ((X Caixa Tipo),Origem) -> X (Caixa,Origem) ()
calcOrigins ((Unid a),orig) = Unid (a,orig)
calcOrigins ((Comp t a1 a2),orig) = (Comp () (calcOrigins (a1,orig)) (calcOrigins (a2,calc t orig (dimen a1))))

calc :: Tipo -> Origem -> (Float, Float) -> Origem
calc t (a,b) (x,y) |t==Ht = (a+x,b+y)
                       |t==Vd = (a+x,b+y)
                       |t==H = (a+x,b+y/2)
                       |t==V = (a+x/2,b+y)
                       |t==Ve = (a,b+y)
...
agrup_caixas :: X (Caixa,Origem) () -> Fig
agrup_caixas (Unid a) = singl(swap a)
agrup_caixas (Comp t a1 a2) = (agrup_caixas a1) ++ (agrup_caixas a2)

mostra_caixas :: (L2D , Origem ) -> IO ()
mostra_caixas f = display (caixasAndOrigin2Pict f)

caixasAndOrigin2Pict :: (L2D , Origem ) -> G.Picture
caixasAndOrigin2Pict f =  G.pictures(toGPictures(agrup_caixas (calcOrigins f)))

toGPictures::Fig->[G.Picture]
toGPictures = map f
  where  f (orig, a) = (crCaixa orig (fromIntegral (p1(p1(a)))) (fromIntegral (p2(p1(a)))) (p1(p2(a))) (p2(p2(a))))
\end{code}
...
cos' x = prj . for loop init where
   loop(c,h,s,f) = (h+c,(((-1)*x*x)/s)*h,f+s,8+f)
   init = (1,(-x*x)/2,12,18)
   prj(c,h,s,f)  = c
\end{code}
...
check = cataFS (nr . map (p1))

tarAux :: (a,[([a],b)]) -> [([a], b)]
tarAux (a, []) =[]
tarAux (a, (l,b):xs) = (a:l,b):(tarAux(a,xs))

tar :: FS a b -> [(Path a, b)]
tar = cataFS( concat . map ((either ( singl . (singl >< id)) (tarAux) ) . distr))

addFile :: (([a],b), FS a b) -> FS a b
addFile (([x],b), FS l) = FS ((x,File b):l)
addFile (((h:t),b), FS l) = FS (singl(h, Dir (addFile ((t,b), FS l))))

untarAux :: () -> FS a b
untarAux () = FS []

untar :: (Eq a) => [(Path a, b)] -> FS a b
untar = joinDupDirs . cataList(either (untarAux) addFile)

findAux1 :: (Eq a) => a -> (a,b) -> [[a]]
findAux1 x (a,b) = if (x==a) then singl (singl a) else nil (nil)

findAux2 :: (Eq a) => a -> (a, [[a]]) -> [[a]]
findAux2 l (x,(h:t)) = if (elem l h) then (x:h) : (findAux2 l (x,t)) else findAux2 l (x,t)

find :: (Eq a) => a -> FS a b -> [Path a]
find nome = cataFS(concat.map((either (findAux1 nome) (findAux2 nome)).distr))
